...
@section('content')
    <form method="POST" action="{{ route('installers.welcome.next') }}" id="welcome-form">
        ...
        <x-core::form.select :label="trans('packages/installer::installer.welcome.language')" name="language" :options="$languages" :value="old('language', app()->getLocale())" />
    </form>
@endsection

@section('footer')
    <x-core::button type="submit" color="primary" form="welcome-form">
